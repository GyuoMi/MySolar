CREATE OR REPLACE FUNCTION calculate_monthly_totals(id INT)
RETURNS JSON AS $$
DECLARE
    ...
    total_usage FLOAT;
BEGIN
    -- Calculate total_production in kW hours for the past 30 days based on records_tbl
    ...
    WHERE records_tbl.user_id = id AND 
          TO_TIMESTAMP(records_tbl.records_time, 'YYYY-MM-DD HH24:MI:SS') >= NOW() - INTERVAL '30 days' AND 
          device_tbl.device_usage = true;

    -- Calculate total_grid and total_solar for the past 30 days based on device_tbl
    ...
    INNER JOIN device_tbl ON records_tbl.device_id = device_tbl.device_id
    WHERE records_tbl.user_id = id AND 
          TO_TIMESTAMP(records_tbl.records_time, 'YYYY-MM-DD HH24:MI:SS') >= NOW() - INTERVAL '30 days';
