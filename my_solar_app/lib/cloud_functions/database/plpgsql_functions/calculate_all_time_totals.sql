CREATE OR REPLACE FUNCTION calculate_all_time_totals(id INT)
RETURNS JSON AS $$
DECLARE
    ...
    total_usage FLOAT;
BEGIN
    -- Calculate total_production in kW hours based on records_tbl
    ...
    WHERE records_tbl.user_id = id AND device_tbl.device_usage = true;

    -- Calculate total_grid and total_solar based on device_tbl
    ...
    -- Insert the calculated values into the "totals_tbl" table
    INSERT INTO totals_tbl (user_id, total_usage, total_production, total_grid, total_solar)
    VALUES (id, total_grid + total_solar, total_production, total_grid, total_solar);
